...
// Interface para usuário sem código de ativação
interface User {
  id: string
  username: string
  email: string
...
export default function EventControlPro() {
  ...
  const findUser = async (username: string, email?: string): Promise<User | null> => {
    try {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const existingUser = await findUser(loginData.username, loginData.email)
      if (existingUser) {
      ...
      if (error) {
        ...
        if (error.code === '42P01') {
          setLoginError('Tabela de usuários não encontrada. Criando automaticamente...')
          // Tentar criar a tabela e repetir a operação
          await createUsersTable()
          return handleRegister() // Tentar novamente
        } else if (error.code === '23505') {
          setLoginError('Usuário ou email já existe')
        } else if (error.message) {
          setLoginError(`Erro: ${error.message}`)
        } else {
          setLoginError('Erro ao criar conta. Verifique sua conexão e tente novamente.')
        }
        return
      }

      if (!data || data.length === 0) {
        setLoginError('Erro ao criar conta. Dados não foram salvos.')
        return
      }
      ...
      if (error?.message) {
        ...
        setLoginError('Erro inesperado ao criar conta. Tente novamente.')
      }
    } finally {
      setIsLoading(false)
    }
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const user = await findUser(loginData.username)
  ...
  const handleLogout = async () => {
    // Se for usuário do Google, fazer logout do Supabase Auth
  ...
  // Verificar sessão do Google ao carregar
  useEffect(() => {
    const checkGoogleSession = async () => {
      ...
      if (session?.user) {
        ...
        localStorage.setItem('eventcontrol-current-user', JSON.stringify(googleUser))
        
        addNotification('success', 'Login Google', `Bem-vindo, ${googleUser.username}!`)
        return
      }
        ...
        <div className="max-w-md mx-auto p-6 relative z-10 w-full">
          <div className={`${darkMode ? 'bg-gray-800/90' : 'bg-white/90'} backdrop-blur-xl rounded-3xl p-8 shadow-2xl border border-white/20`}>
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full mb-6 py-3 px-4 bg-white border border-gray-300 rounded-xl hover:bg-gray-50 transition-all duration-300 hover:scale-105 shadow-lg hover:shadow-xl flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className={`text-xs sm:text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                  Logado como: {currentUser?.username} {currentUser?.provider === 'google' && '(Google)'}
                </p>
              </div>
            </div>
